...
export default function DetectionPage({ onResultReceived }) {
  ...
  const handleFileSelect = (e) => { const file = e.target.files[0]; if (file) validateAndSetFile(file) }
  const handleDragOver = (e) => { e.preventDefault(); setIsDragging(true) }
  const handleDragLeave = () => setIsDragging(false)
  const handleDrop = (e) => { e.preventDefault(); setIsDragging(false); const file = e.dataTransfer.files[0]; if (file) validateAndSetFile(file) }

  const validateAndSetFile = (file) => {
    setError('')
    const allowedTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/webp']
    if (!allowedTypes.includes(file.type)) { setError('Please upload a valid image (JPG, PNG, or WebP)'); return }
  ...
  const handleAnalyze = async () => {
    ...
    setIsLoading(true); setError(''); setProgress(10)

    try {
      const formData = new FormData()
      formData.append('file', selectedFile)

      const progressInterval = setInterval(() => { setProgress(prev => (prev >= 90 ? 90 : prev + 10)) }, 200)
      ...
      if (response.data && response.data.prediction) {
        ...
        const mappedResult = {
          success: true,
          prediction: {
            class: raw.prediction.disease,
            confidence: Math.round((raw.prediction.confidence || 0) * 100),
            category: raw.prediction.is_healthy ? 'Healthy' : (raw.prediction.disease.includes('Deficiency') ? 'Nutrient Deficiency' : 'Disease'),
            symptoms: raw.prediction.symptoms || [],
            treatment: raw.recommendations?.remedies || [],
            prevention: raw.recommendations?.nutrients || [],
            description: `Detected ${raw.prediction.disease} with confidence ${Math.round((raw.prediction.confidence || 0) * 100)}%.`
          },
          top_predictions: (raw.top_predictions || []).map(tp => ({
            class: tp.disease,
            confidence: Math.round((tp.confidence || 0) * 100)
          }))
        }
      ...
      if (err.response) {
        if (err.response.status === 413) setError('File too large. Please upload an image smaller than 16MB')
        else if (err.response.status === 400) setError('Invalid file type. Please upload JPG, PNG, or JPEG')
      ...
      } else if (err.request) setError('No response from server. Please check if the backend is running.')
  ...
  const handleReset = () => { setSelectedFile(null); setPreviewUrl(''); setError(''); setProgress(0); if (fileInputRef.current) fileInputRef.current.value = '' }
  const handleCameraCapture = () => { alert('Camera capture coming soon! Please upload an image for now.') }
      ...
      <div className="grid md:grid-cols-2 gap-10">
        <div>
          <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
            ...
            <div
              className={`border-3 border-dashed rounded-xl p-10 text-center cursor-pointer transition-all duration-300 mb-6 ${isDragging ? 'border-green-500 bg-green-50 scale-105' : 'border-green-300 hover:border-green-400 hover:bg-green-50'} ${previewUrl ? 'hidden' : ''}`}
            ...
            <div className="flex space-x-4">
              ...
              <button onClick={handleAnalyze} disabled={!selectedFile || isLoading} className={`flex-1 font-medium py-3 px-4 rounded-lg transition flex items-center justify-center ${!selectedFile || isLoading ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700 text-white transform hover:scale-105'}`}>
                {isLoading ? (<><span className="animate-spin rounded-full h-5 w-5 border-b-2 border-white mr-2"></span>Analyzing...</>) : (<><span className="mr-2">🔬</span>Analyze Leaf</>)}
              </button>
            </div>

            {error && (<div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-lg"><div className="flex items-center text-red-700"><span className="mr-2">⚠️</span><p className="font-medium">{error}</p></div></div>)}
          </div>
        </div>
